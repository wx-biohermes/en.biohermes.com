...
//初始化组件
function fileupload_file_init(json){
    $("#fileupload_"+json.name).fileupload({
        ...
        progressall: function (e, data) {
            ...
            $("#fileupload_"+json.name+" .fileupload-progress").show();
            $("#fileupload_"+json.name+" .fileupload-progress").removeClass("fade");
            $("#fileupload_"+json.name+" .progress-bar-success").attr("style", "width: "+progress+"%");
        },
        add: function (e, data) {
                ...
                $.ajax({type: "POST",dataType:"json", url: url, data: $(body).find('#myform').serialize(),
                    success: function(json2) {
                        ...
                        if (json2.code == 1) {
                            ...
                            html+= tpl;
                            $('#fileupload_' +json.name +'_files').html(html);
                            $('#fileupload_'  +json.name ).find('.fileinput-delete').show();
                            dr_tips(1, json2.msg);
                        } else {
                            dr_tips(0, json2.msg);
...
function fileupload_files_init(json) {

    $("#fileupload_"+json.name+"_files").sortable();
    dr_slimScroll_init(".scroller_"+json.name+"_files", 300);
    // 初始化上传组件
    $("#fileupload_"+json.name).fileupload({
        disableImageResize: false,
        autoUpload: true,
        ...
        add: function (e, data) {
            ...
            var numItems = $("#fileupload_"+json.name+"_files .files_row").length;
            if(numItems + myItems > json.count){
                dr_tips(0, json.error);
                return false;
            }
            data.submit();
        },
            ...
            tpl = tpl.replace(/\{upload\}/g, "<input type=\"file\" name=\"file_data\"></button>");
            $("#fileupload_"+json.name+"_files").append(tpl);

            dr_slimScroll_init(".scroller_"+json.name+"_files", 300);
            //fileupload_file_edit();
        },
        fail: function (e, data) {
            //console.log(data.errorThrown);
            dr_tips(0, dr_lang("上传请求错误：")+data.errorThrown);
    ...
    // 修改组件
    $('#fileupload_'+json.name+'_files .file_edit').fileupload({
        disableImageResize: false,
        autoUpload: true,
        maxFileSize: json.size,
        acceptFileTypes: json.ext,
                ...
                $.ajax({type: "POST",dataType:"json", url: url, data: $(body).find('#myform').serialize(),
                    success: function(json2) {
                        ...
                        if (json2.code == 1) {
                            ...
                            var numItems = $('#fileupload_'+json.name+'_files .files_row').length;
                            if(numItems + json2.data.count > numCount){
                                dr_tips(0, json.error);
                                return false;
                            }
                            var html = "";
                            ...
                            $('#fileupload_'+json.name+'_files').append(html);
                            dr_slimScroll_init('.scroller_'+json.name+'_files', 300);
                            dr_tips(1, json2.msg);
                        } else {
                            dr_tips(0, json2.msg);
                ...
                $.ajax({type: "POST",dataType:"json", url: json.input_url, data: $(body).find('#myform').serialize(),
                    success: function(json2) {
                        ...
                        if (json2.code == 1) {
                            ...
                            tpl = tpl.replace(/\{preview\}/g, json2.data.preview);
                            tpl = tpl.replace(/\{id\}/g, json2.data.id);
                            tpl = tpl.replace(/\{disabled\}/g, json2.data.disabled);
                            tpl = tpl.replace(/\{filepath\}/g, json2.data.file);
                            tpl = tpl.replace(/\{title\}/g, json2.data.name);
                            tpl = tpl.replace(/\{description\}/g, "");
                            tpl = tpl.replace(/\{upload\}/g, json2.data.upload);
...
// 修改URL
function fileupload_file_edit(name, e)
{

    var json = eval('files_json_' +name);
