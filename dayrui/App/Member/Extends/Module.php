            ...
            return parent::_Save($id, $data, $old,
                ...
                function ($id, $data, $old) {
                    ...
                    if ($old) {
                        ...
                        $type_post = \Phpcmf\Service::L('member_auth', 'member')->category_auth($this->module, $data[1]['catid'], 'type_post', $this->member);
                        if ($this->uid) {
                            // 判断日发布量
                            $day_post = \Phpcmf\Service::L('member_auth', 'member')->category_auth($this->module, $data[1]['catid'], 'day_post', $this->member);
                            if ($day_post) {
                                if (\Phpcmf\Service::L('member_auth', 'member')->is_category_public && $type_post) {
                                    // 按全局算
                            ...
                            // 判断发布总量
                            $total_post = \Phpcmf\Service::L('member_auth', 'member')->category_auth($this->module, $data[1]['catid'], 'total_post', $this->member);
                            if ($total_post) {
                                if (\Phpcmf\Service::L('member_auth', 'member')->is_category_public && $type_post) {
                                    // 按全局算
                                    if (\Phpcmf\Service::M()->db
                            ...
                            // 金币验证
                            $score = \Phpcmf\Service::L('member_auth', 'member')->category_auth($this->module, $data[1]['catid'], 'score', $this->member);
                            ...
                            // 金额验证
                            $money = \Phpcmf\Service::M('member_auth', 'member')->category_auth($this->module, $data[1]['catid'], 'money', $this->member);
