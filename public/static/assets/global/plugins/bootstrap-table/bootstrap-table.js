    ...
    var EN = {
        ...
        formatShowingRows: function formatShowingRows(pageFrom, pageTo, totalRows, totalNotFiltered) {
            if (totalNotFiltered !== undefined && totalNotFiltered > 0 && totalNotFiltered > totalRows) {
                return dr_lang("显示第").concat(pageFrom, dr_lang("到第")).concat(pageTo, dr_lang("条记录，总共")).concat(totalRows, dr_lang("条记录（从")).concat(totalNotFiltered, dr_lang("总记录中过滤）"));
            }

            return dr_lang("显示第").concat(pageFrom, dr_lang("到第")).concat(pageTo, dr_lang("条记录，总共")).concat(totalRows, dr_lang("条记录"));
        },
        formatSRPaginationPreText: function formatSRPaginationPreText() {
            return dr_lang('上一页');
        },
        formatSRPaginationPageText: function formatSRPaginationPageText(page) {
            return dr_lang("第").concat(page, dr_lang("页"));
        },
        formatSRPaginationNextText: function formatSRPaginationNextText() {
            return dr_lang('下一页');
        },
        formatDetailPagination: function formatDetailPagination(totalRows) {
            return dr_lang("总共").concat(totalRows, dr_lang("条记录"));
        },
        ...
        formatFilterControlSwitch: function formatFilterControlSwitch() {
            return dr_lang('隐藏/显示过滤控制');
        },
        formatFilterControlSwitchHide: function formatFilterControlSwitchHide() {
            return dr_lang('隐藏过滤控制');
        },
        formatFilterControlSwitchShow: function formatFilterControlSwitchShow() {
            return dr_lang('显示过滤控制');
        }
    };
    var COLUMN_DEFAULTS = {
        field: undefined,
            ...
            value: function initHeader() {
                ...
                this.options.columns.forEach(function (columns, i) {
                    ...
                    columns.forEach(function (column, j) {
                        ...
                        style += Utils.sprintf('width: %s; ', (column.checkbox || column.radio) && !width ? !column.showSelectTitle ? '54px' : undefined : width ? width + unitWidth : undefined);
...
(function ($) {
    ...
    BootstrapTable.prototype.initPagination = function() {
        ...
        if(this.options.paginationShowPageGo){
            this.$pagination.find('ul.pagination').append('<li class="input-page"><div class="input-group">'+
                '<input type="text" value="' + this.options.pageNumber + '" class="form-control page-input">'+
            '<span class="input-group-btn page-go">'+
            '<button class="btn jump-go" type="button">'+dr_lang('跳转')+'</button>'+
                '</span></div></li>');
...
// 批量删除确认
function dr_table_option(url, msg) {
            ...
            $.ajax({
                ...
                success: function(json) {
                    ...
                    if (json.token) {
                        ...
                        post_token[token.name] = token.value;
                    }
                    if (json.code) {
